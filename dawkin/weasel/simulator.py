...
class Simulator:
    ...
    def mutate(self, str):
        ...
        for key in range(len(str)):
            if self._mutation_rate >= random.random():
                str[key] = self._characters[random.randint(0, len(self._characters) - 1)]
            #endif    
        #end_for
    ...
    def run(self, target):
        ...
        current = ''.join([self._characters[random.randint(0, len(self._characters) - 1)] for i in range(len(target))]) 
        iterations = 0
