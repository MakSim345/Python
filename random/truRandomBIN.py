...
# proxies = {'http': 'http://3.249.252.161:88'}
# http://gems.setpac.ge.com/pac.pac
# PROXY: 3.187.4.17:88
# PROXY: 3.249.252.161:88
# http://gems.setpac.ge.com:1533/pac.pac
...
def take_true_random_number_39():
    ...
    try:
        ...
        print "ERROR in 39 - no proxy in use"
    
    try:
        count = 1# int(argv[1])
        true_rnd = urllib2.urlopen("http://random.org/integers/?num=1&min=1&max=39&col=1&base=10&format=plain&rnd=new", proxies).read()
    except:
        print "ERROR in 39 - with proxy"
...
def lotto_7x39_number(number = 7):
    ...
    for i in range(number):
        _origin = True
        while _origin:
            _tmp_number = int (take_true_random_number_39())
            #print _tmp_number
            if _tmp_number in a:
                #print "Incorrect:", _tmp_number, "is in array.", a
                _origin = True
