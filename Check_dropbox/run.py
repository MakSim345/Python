...
if __name__ == "__main__":
    ...
    # search_path_main_box = "D:/Dropbox"

    #select correct path to Win32 Dropbox:
    #if not os.path.exists(search_path_main_box):
    #    sys.stderr.write("Info: '"+self.filename+"' does not exist. Nothing to do\n")    
    #else:        
    #    sys.stderr.write("Info: '"+self.filename+"' does exist.\n")
    # endif 

    _result_to_send = time.strftime("%Y-%m-%d %H:%M:%S\n")
    _sd = search_dublicates(search_path_main_box)
    _sd.run(NO_ERASE_FILES)
    _result_to_send = _result_to_send + time.strftime("%Y-%m-%d %H:%M:%S\n")

    _result_to_send = _result_to_send + _sd.compare_dictionaryes()

    sender =  sendSMS('YANDEX', 'Dropbox daily')
