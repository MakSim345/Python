...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            canvas.draw_image(self.image, [((self.age % self.lifespan) // 1 ) * self.image_size[0] + self.image_center[0], self.image_center[1]], self.image_size, self.pos, self.image_size, self.angle)
        else:
...
#    a_missile.update()
    ...
    if lives == 0:
        started = False
        rock_group.intersection_update(missile_group)
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        if dist(a_rock.get_position(), my_ship.get_position()) > (10 + a_rock.get_radius() + my_ship.get_radius()):
...
def group_group_collide(group1, group2):
    ...
    for element in group1:
        if group_collide(group2, element):
            to_remove1.add(element)
    for element in to_remove1:
